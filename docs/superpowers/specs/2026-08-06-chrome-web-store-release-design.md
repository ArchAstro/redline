...
`http://127.0.0.1:7878/connect#pair=<secret>&expires_at=<canonical-expiry>` directly, so the fragment is
...
atomically advances that generation. It retains a content-free, token-bound
clear receipt for 30 days so a browser retry after token revocation can prove
the clear committed without treating an unrelated authorization failure as
success. Re-pairing cannot submit a retained draft
